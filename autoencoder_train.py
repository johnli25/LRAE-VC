...
def eval_autoencoder(model, dataloader, criterion, device, max_tail_length=None, quantize=0):
    ...
    with torch.no_grad():
        for inputs, _ in dataloader:
            ...
            end_time = time.time()
            print(f"Time taken for whole batch of size {inputs.size(0)} frames: {end_time - start_time:.4f} seconds")
...
if __name__ == "__main__":
    ...
    #     print(f"Final Test Loss: {final_test_loss:.6f} and PSNR: {final_test_psnr:.6f}")
