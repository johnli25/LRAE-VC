...
def transmission_test():
    ...
    try:
        ...
        # receiver.cmd("bash ~/receiver.sh > /tmp/receiver_log.txt 2>&1 &")
        receiver.cmd("stdbuf -oL bash ~/receiver.sh > /tmp/receiver_log.txt 2>&1 &")
        time.sleep(2)  

        print(receiver.cmd("cat /tmp/receiver_log.txt"))
        ...
        sender.cmd("bash ~/sender.sh > /tmp/sender_log.txt 2>&1 &")
